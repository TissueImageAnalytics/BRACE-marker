...
##### usage of the above function (forest_plot) for generating forest plots ######@@@@@@@@@@@@@@@
##the function forest_plot is expecting the event value=1 for an event and anything else will be set to 0 i.e. no event

work_dir = 'D:/warwick/tasks/Nottingham_Exemplar/ML/cellular_analysis/R' ##path where the code and other files are located
cohorts <- list("Cohort-B") ## <"Cohort-A"|"Cohort-B">
...
plot_width = 10 ##width of the plot when saving
plot_height = 5 ##width of the plot when saving

for (c in cohorts){
  ## feature file along with time/event. these files can be generated by python script at /survival_analysis/KM_curves.py
  ## the files can be put in the current working directory as set above work_dir.
  
  csv_name = paste0(c, '_for_forest_plots.csv')
